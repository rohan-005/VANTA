...
export const SystemCard: React.FC<SystemCardProps> = ({
  ...
  return (
    <div
      ...
      className={`relative w-full bg-vanta-charcoal border ${
        isSelected || isPro
          ? 'border-vanta-lime shadow-glow-lime-sm'
          : 'border-vanta-border hover:border-vanta-lime/40'
      } rounded-sm p-6 flex flex-col justify-between transition-all duration-300 group cursor-pointer`}
    >
      {/* Featured Badge */}
      {system.badge && (
        <div className="absolute -top-3 left-6 px-3 py-0.5 bg-vanta-lime text-vanta-dark font-mono text-[10px] font-bold uppercase tracking-widest rounded-sm shadow-md">
          {system.badge}
        </div>
      )}
      ...
      <div>
        ...
        <div className="relative w-full aspect-[16/10] bg-vanta-dark border border-vanta-border rounded-sm p-4 mb-6 overflow-hidden flex items-center justify-center group-hover:border-vanta-lime/40 transition-colors">
          ...
          <div className="relative z-10 flex flex-col items-center justify-center text-center space-y-2">
            <div className={`w-20 h-24 border-2 ${isSelected || isPro ? 'border-vanta-lime' : 'border-vanta-border'} bg-vanta-charcoal rounded-sm relative flex flex-col items-center justify-center p-2 shadow-xl`}>
        ...
        <div className="space-y-1 mb-4">
          <div className="flex items-center justify-between">
            <h3 className="text-2xl font-display font-extrabold uppercase tracking-wide text-vanta-ivory group-hover:text-vanta-lime transition-colors">
              {system.name}
            </h3>
            <span className="font-mono text-xs text-vanta-text-muted-light uppercase font-semibold">
              {system.category} TIER
            </span>
          </div>
          <p className="font-sans text-xs text-vanta-lime font-bold">
            {system.tagline}
          </p>
        </div>

        {/* Description */}
        <p className="font-sans text-xs text-vanta-text-muted-light leading-relaxed mb-6">
          {system.description}
        </p>

        {/* Key Hardware Specs Breakdown */}
        <div className="space-y-2 py-4 border-t border-b border-vanta-border mb-6 font-mono text-xs">
          <div className="flex justify-between items-center py-1">
            <span className="text-vanta-text-muted-light text-[11px]">PROCESSOR</span>
            <span className="text-vanta-ivory text-[11px] font-semibold truncate max-w-[200px]" title={system.specifications.cpu}>
              {system.specifications.cpu.split('(')[0]}
            </span>
          </div>

          <div className="flex justify-between items-center py-1 border-t border-vanta-border/40">
            <span className="text-vanta-text-muted-light text-[11px]">GRAPHICS</span>
          ...
          <div className="flex justify-between items-center py-1 border-t border-vanta-border/40">
            <span className="text-vanta-text-muted-light text-[11px]">MEMORY</span>
            <span className="text-vanta-ivory text-[11px] font-medium">
              {system.specifications.memory.split(' ')[0]} {system.specifications.memory.split(' ')[1]}
            </span>
          </div>

          <div className="flex justify-between items-center py-1 border-t border-vanta-border/40">
            <span className="text-vanta-text-muted-light text-[11px]">STORAGE</span>
            <span className="text-vanta-ivory text-[11px] font-medium">
        ...
        <div className="space-y-1.5 mb-6">
          {system.highlights.map((item, idx) => (
            <div key={idx} className="flex items-center gap-2 font-sans text-[11px] text-vanta-text-muted-light">
      ...
      <div className="space-y-4 pt-4 border-t border-vanta-border">
        <div className="flex items-baseline justify-between">
          <span className="font-mono text-xs text-vanta-text-muted-light">STARTING AT</span>
          <span className="font-display font-extrabold text-2xl text-vanta-ivory">
        ...
        <div className="grid grid-cols-2 gap-2" onClick={(e) => e.stopPropagation()}>
          <Button
            variant={isSelected || isPro ? "primary" : "dark"}
            size="sm"
          ...
          <Button
            ...
            className="w-full text-[11px] border-vanta-border hover:border-vanta-lime hover:text-vanta-lime text-vanta-ivory"
          >
            FULL SPECS
          </Button>
        </div>
