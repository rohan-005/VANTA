    ...
    >
      ...
      <div className="w-full gutter-padding relative z-10">
        ...
        <div className="flex flex-col items-start space-y-4 mb-12">
          ...
          <p className="text-base text-vanta-text-muted-light font-sans max-w-2xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2 p-1.5 bg-vanta-charcoal border border-vanta-border rounded-sm mb-10">
          {SHOWCASE_TABS.map((tab) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-start p-3.5 sm:p-4 rounded-sm transition-all text-left focus:outline-none focus-visible:ring-1 focus-visible:ring-vanta-lime ${
                  isActive
                    ? 'bg-vanta-dark border border-vanta-lime text-vanta-ivory shadow-lg'
                    : 'bg-transparent border border-transparent text-vanta-text-muted-light hover:text-vanta-ivory hover:bg-vanta-charcoal/50'
                }`}
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-stretch">
          ...
          <div className="lg:col-span-6 bg-vanta-charcoal border border-vanta-border p-6 sm:p-8 rounded-sm flex flex-col justify-between space-y-6">
            <div>
              <div className="inline-block px-3 py-1 bg-vanta-lime-dim border border-vanta-lime/40 text-vanta-lime font-mono text-[11px] uppercase tracking-widest rounded-sm mb-4 font-bold">
                {activeTab.specPill}
              </div>

              <h3 className="text-2xl sm:text-3xl font-display font-bold uppercase tracking-wide text-vanta-ivory leading-snug">
                {activeTab.headline}
              </h3>
            ...
            <div className="space-y-4 pt-4 border-t border-vanta-border">
              {activeTab.features.map((feature: ShowcaseFeature, idx: number) => (
                <div
                  key={idx}
                  className="p-4 bg-vanta-dark border border-vanta-border hover:border-vanta-lime/50 rounded-sm transition-colors flex items-start gap-4"
                >
                  <div className="p-2 bg-vanta-charcoal border border-vanta-border rounded-sm shrink-0 mt-0.5">
                    {getIcon(feature.iconName)}
                  </div>
                  <div className="flex-1">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-xs uppercase tracking-wider text-vanta-text-muted-light">
                        {feature.title}
                      </span>
                    ...
                    <p className="font-sans text-xs text-vanta-ivory mt-1">
                      {feature.detail}
                    </p>
                  </div>
                </div>
          ...
          <div className="lg:col-span-6 bg-vanta-charcoal border border-vanta-border p-6 sm:p-8 rounded-sm flex flex-col justify-between relative overflow-hidden">
            ...
            <div className="flex items-center justify-between pb-4 border-b border-vanta-border">
              <div className="flex items-center gap-2">
                ...
                <span className="font-mono text-xs uppercase tracking-widest text-vanta-ivory font-semibold">
            ...
            <div className="py-8 my-auto flex flex-col items-center justify-center relative min-h-[300px]">
              ...
              {/* Domain Specific Interactive Render Graphics (NO BLUE) */}
              {activeTab.id === 'performance' && (
                <div className="w-full flex flex-col items-center space-y-6">
                  <div className="w-48 h-48 bg-vanta-dark border-2 border-vanta-lime rounded-sm relative flex items-center justify-center shadow-glow-lime">
                    ...
                    <div className="absolute top-2 left-2 font-mono text-[9px] text-vanta-lime font-bold">
                      SILICON_BENCH
                    </div>
                    <div className="absolute bottom-2 right-2 font-mono text-[9px] text-vanta-text-muted-light">
                      LGA-1700 / AM5
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3 w-full max-w-md">
                    <SpecPill label="GPU BUS" value="PCIe 5.0 x16" highlight theme="dark" />
                    <SpecPill label="VRAM TEMP" value="62°C Peak" theme="dark" />
                  </div>
                </div>
              )}
              ...
              {activeTab.id === 'cooling' && (
                <div className="w-full flex flex-col items-center space-y-6">
                  <div className="w-full max-w-md h-48 bg-vanta-dark border border-vanta-lime/60 rounded-sm relative p-4 flex flex-col justify-between">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-xs text-vanta-lime font-bold">DUAL 360mm RADIATORS</span>
                    ...
                    <div className="flex items-center justify-center gap-6">
                      <div className="p-3 bg-vanta-charcoal border border-vanta-border rounded-sm text-center">
                        <span className="block font-mono text-xs text-vanta-text-muted-light">INLET TEMP</span>
                        <span className="font-mono text-lg font-bold text-vanta-lime">22.4°C</span>
                      </div>
                      <div className="text-vanta-orange">→</div>
                      <div className="p-3 bg-vanta-charcoal border border-vanta-border rounded-sm text-center">
                        <span className="block font-mono text-xs text-vanta-text-muted-light">EXHAUST TEMP</span>
                  ...
                  <div className="grid grid-cols-2 gap-3 w-full max-w-md">
                    <SpecPill label="COOLANT" value="Dielectric Fluid" highlight theme="dark" />
                    <SpecPill label="FAN DENSITY" value="7x High Pressure" theme="dark" />
                  </div>
                </div>
              )}
                ...
                <div className="w-full flex flex-col items-center space-y-6">
                  <div className="w-full max-w-md h-48 bg-vanta-dark border border-vanta-border rounded-sm relative p-4 flex flex-col justify-between">
                    ...
                    <div className="grid grid-cols-3 gap-2 py-4">
                      <div className="border border-vanta-border p-2 text-center bg-vanta-charcoal">
                  ...
                  <div className="grid grid-cols-2 gap-3 w-full max-w-md">
                    <SpecPill label="FRAME MATERIAL" value="CNC Billet Alloy" highlight theme="dark" />
                    <SpecPill label="CABLE ROUTING" value="35mm Rear Tunnel" theme="dark" />
                  </div>
                </div>
              )}
              ...
              {activeTab.id === 'craft' && (
                <div className="w-full flex flex-col items-center space-y-6">
                  <div className="w-full max-w-md h-48 bg-vanta-dark border border-vanta-lime/40 rounded-sm relative p-4 flex flex-col justify-between">
                    <div className="flex items-center justify-between">
                      <span className="font-mono text-xs text-vanta-lime font-bold">QC CERTIFICATION PASSED</span>
                    ...
                    <div className="space-y-2 py-2">
                      <div className="flex justify-between font-mono text-xs text-vanta-text-muted-light">
                        ...
                        <span className="text-vanta-lime font-semibold">PASSED // STABLE</span>
                      </div>
                      <div className="flex justify-between font-mono text-xs text-vanta-text-muted-light">
                        <span>PRIME95 CPU STRESS (24H)</span>
                        <span className="text-vanta-lime font-semibold">PASSED // NO THROTTLE</span>
                      </div>
                      <div className="flex justify-between font-mono text-xs text-vanta-text-muted-light">
                  ...
                  <div className="grid grid-cols-2 gap-3 w-full max-w-md">
                    <SpecPill label="BUILD OWNER" value="Single Technician" highlight theme="dark" />
                    <SpecPill label="CRATE TYPE" value="Industrial Flight Case" theme="dark" />
                  </div>
                </div>
              )}
